...
codex exec --approve-for-me -C "$ROOT" -m "$model" \
  "Digest paper $ck into this knowledge base. Follow meta/AGENT-TASK.md exactly, processing ONLY citekey $ck. Stop after step 7 of the per-paper procedure. Do NOT open an xreview/ai-review round for this — the verification gate is the main session's job, not yours." \
  > "$ROOT/meta/agent-logs/$ck.log" 2>&1
rc=$?
if [ "$rc" -ne 0 ]; then

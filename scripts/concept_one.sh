...
"$GROK" --cwd "$ROOT" -m "$model" --no-plan --always-approve --max-turns 120 \
  ...
  > "$ROOT/meta/agent-logs/concept-$slug.json" 2>"$ROOT/meta/agent-logs/concept-$slug.err"
if [ -s "$ROOT/concepts/$slug.md" ]; then
  echo "DONE  concept:$slug  $(date +%H:%M:%S)  $(python3 "$ROOT/scripts/_usage_line.py" "$ROOT/meta/agent-logs/concept-$slug.json" 2>/dev/null)"
else
